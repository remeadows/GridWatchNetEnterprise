 ...
 *
 * Usage: node tests/benchmarks/ipam.benchmark.js
 *
 * Prerequisites:
 * - Gateway running on localhost:3001
 * - PostgreSQL running with IPAM schema
...
const BENCHMARK_CONFIG = {
  ...
  listAddresses: {
    url: "/api/v1/ipam/addresses?limit=100",
    method: "GET",
    duration: 10,
    connections: 30,
    pipelining: 3,
  ...
  searchAddresses: {
    url: "/api/v1/ipam/addresses/search?q=192.168",
    method: "GET",
    duration: 10,
    connections: 20,
    pipelining: 2,
    target: {
      rps: 200,
      p99: 300, // ms - search can be slower
    },
    requiresAuth: true,
  },
  listDevices: {
    url: "/api/v1/ipam/devices",
    method: "GET",
    duration: 10,
    connections: 50,
    pipelining: 5,
...
async function main() {
  ...
  if (!authenticated) {
    console.error(
      "\n❌ Error: Could not authenticate. All IPAM benchmarks require authentication.",
    );
    process.exit(1);
  }
